...
example : (-1) ∉ PositiveIntegers := by
  ...
  -- grind -- a nice tactic that can prove these "linear (in)equalities"
...
example (α : Type) (S : Set α) (P : S → Prop) (x : ↑S) (hx : x ∈ subsub P) : {s : S // P s} := by
  ...
  exact hx
-- or as follows
example (α : Type) (S : Set α) (P : S → Prop) (x : ↑S) (hx : x ∈ subsub P) : x.1 ∈ S := by
...
example (α β : Type) (f : α → β) (S : Set α) : S ≠ ∅ → f '' S ≠ ∅ := by
  intro hS hfS
  apply hS
  rw [eq_empty_iff_forall_notMem] at hfS ⊢
  intro x hx
  replace hfS := hfS (f x)
  apply hfS
  use x
...
example : InjOn (fun n : ℤ ↦ n ^ 2) PositiveIntegers := by
  ...
  exact eq_of_le_of_not_lt (le_of_not_gt h_nlt) h_mlt
...
example (N : OddNaturals) :  N.1 ∈ Nat.succ ⁻¹' (EvenNaturals) := by
  ...
  · replace hnz := Nat.exists_eq_succ_of_ne_zero hnz
    ...
    rw [hm] at hn'
    rw [Nat.succ_mod_two_eq_one_iff] at hn'
    change n.succ % 2 = 0 --`show` changes the goal to something definitionally equivalent
    grind
    -- *Alternative proof* if you forgot about `grind`:
    -- rw [hm]
